...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        q = q.reshape(x.shape[0], x.shape[1], self.num_heads, -1).transpose(1, 2)
        k = k.reshape(x.shape[0], x.shape[1], self.num_heads, -1).transpose(1, 2)
        v = v.reshape(x.shape[0], x.shape[1], self.num_heads, -1).transpose(1, 2)
        
        fa = F.scaled_dot_product_attention(q, k, v)
        fa = fa.transpose(1, 2).reshape(x.shape[0], x.shape[1], self.hidden_size)
        o = self.proj(fa)
...
class MLP(nn.Module):
    ...
    def forward(self, x):
        return self.out_proj(F.silu(self.in_proj(x)) * self.gate_proj(x))
