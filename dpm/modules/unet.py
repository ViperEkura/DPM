...
class AttentionBlock(nn.Module):
    def __init__(self, channels:int, num_heads:int):
        ...
        self.o_proj = nn.Conv2d(channels, channels, kernel_size=1)
    
    def _to_seq(self, x: Tensor):
        # [B, C, H, W] -> [B, C, H*W] -> [B, H*W, C]
        return x.flatten(2).transpose(1, 2)

    def _to_img(self, x: Tensor, H: int, W: int):
    ...
    def forward(self, x: Tensor):
        ...
        q = self._to_seq(self.q_proj(x))
        k = self._to_seq(self.k_proj(x))
        v = self._to_seq(self.v_proj(x))
        o = self._to_img(F.scaled_dot_product_attention(q, k, v), H, W)
        h = self.o_proj(o)
